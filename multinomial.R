...
summary.mnl = NULL
 if(bayesian == TRUE){
   library(UPG)
   Date.temp =  model.frame( formula, Data)
   y =  as.factor(Date.temp[[1]])
   ...
   results.mnl <- UPG(y = y, X = X, type = 'mnl', verbose = TRUE, baseline = ref)
  ...
  coMean = colMeans( (results.mnl$posterior$beta.post))
  co2.5 = colQuantile_2.5(exp(results.mnl$posterior$beta.post))
  co97.5 = colQuantile_97.5(exp(results.mnl$posterior$beta.post))
 ...
 co= (2 %f% coMean)%+% " (" %+%(2 %f% co2.5) %+% ", " %+% (2 %f% co97.5)%+% ")"
   ...
   plot.UPG    = function(x         = NULL,
     ...
     c.point  = apply(x$posterior$beta.post[,include,,drop=F], c(2,3), mean)
     c.upper  = apply(x$posterior$beta.post[,include,,drop=F], c(2,3), quantile, q[2])
     c.lower  = apply(x$posterior$beta.post[,include,,drop=F], c(2,3), quantile, q[1])
     
     
     if(OR == TRUE){
       c.point  = apply(exp(x$posterior$beta.post[,include,,drop=F]), c(2,3), mean)
       c.upper  = apply(exp(x$posterior$beta.post[,include,,drop=F]), c(2,3), quantile, q[2])
       c.lower  = apply(exp(x$posterior$beta.post[,include,,drop=F]), c(2,3), quantile, q[1])
     } 
...
list(results= res,
     predict.for.newdata = pp.write,
     plot = plot,
     data.melt.for.ggplot = lpp,
     model = test,
     bayesian.result = co,
     bayesian.model = list(results.mnl = results.mnl, summary.mnl = summary.mnl ),
     bayesian.plot = bayesian.plot
)
}
cat("
